...
comp = HistComparator(tree, style1 = cms_style, style2 = papas_style,
                      nbin = 300, xmin = 0.5, xmax = 1.5, xvar = 'E_{rec}/E_{gen}',
                      var1 = 'cmsjet_e/gen_jet_e',
                      var2 = 'papasjet_e/gen_jet_e',
                      cut = 'gen_jet_pt>10 && gen_jet_e<100 && abs(gen_jet_eta)>1.479 && abs(gen_jet_eta)<3 && simtrack_len==3')

fitter = Fitter2D('histo2d','E_{rec}/E_{gen}vsE_{gen} barrel', 100, 0, 100, 200, 0, 2)
tree.Project('histo2d','cmsjet_e/gen_jet_e:gen_jet_e','gen_jet_e>1 && gen_jet_e<100 && abs(gen_jet_eta)>1.479 && abs(gen_jet_eta)<3 && simtrack_len==3')
fitter.fit_slices()
can1 = TCanvas()
fitter.h2d.Draw()
